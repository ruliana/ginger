...
public class DuckTypeTest {
	...
	@Test
	public void shouldExecuteDirectMethodsWithSimpleArguments() throws Exception {
		assertEquals(0, new DuckType("test").call("compareTo", "test"));
		assertEquals("testing", new DuckType("test").call("concat", "ing"));
	}
	
	@Test
	public void shouldExecuteDirectMethodsWithCompatibleArguments() throws Exception {
	    assertEquals("toast", new DuckType("test").call("replace", "es", "oas")); // "es" and "oas" can be CharSequence =/
	    assertEquals("T", new DuckType("Test").call("substring", 0, 1)); // 0 and 1 are primitives
	    assertEquals("est", new DuckType("Test").call("substring", 1));
	}
	
	@Test
    public void aliasesShouldWorkToo() throws Exception {
	    assertEquals("est", new DuckType("Test").call("substring", 1)); // No alias
	...
	@Test
    public void bugShouldNotConfuseMethodsWithSameSignatureInDifferentObjects() throws Exception {
    ...
    public void methodCachingShouldMakeDifference() throws Exception {
        ...
        int timesFaster = 10;
        assertTrue("Cached should be at least " + timesFaster +"x faster", elapsedNoCacheTime / elapsedCachedTime > timesFaster);
    }
